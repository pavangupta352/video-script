...
source_name = "YourMediaSourceName"
...
def play_video(video_path, idle=False):
    ...
    if duration == 0:
        print(f"Could not get duration for video: {video_path}. Skipping.")
        return
    ...
    if source:
        ...
        if not idle:  # Only update the last_played_video if it's not an idle video
            last_played_video = video_path
        video_end_time = time.time() + duration
        print(f"[Script] Now playing: {video_path}")
...
# The following functions remain unchanged from your original script.
